...
static void place_water(dungeon *d)
{
  pair_t p;
  do {
    while ((p[dim_y] = rand_range(1, DUNGEON_Y - 2)) &&
...
static void place_earthquack(dungeon *d)
{
  pair_t p;
  do {
    while ((p[dim_y] = rand_range(1, DUNGEON_Y - 2)) &&
...
uint16_t count_water(dungeon *d)
{
  uint32_t x, y;
  uint16_t i;
...
uint16_t count_earthquack(dungeon *d)
{
  uint32_t x, y;
  uint16_t i;
...
int write_water(dungeon *d, FILE *f)
{
  uint16_t num_water;
  uint8_t x, y;

  num_water = htobe16(count_water(d));
  fwrite(&num_water, 2, 1, f);
  for (y = 1; y < DUNGEON_Y - 1 && num_water; y++) {
    for (x = 1; x < DUNGEON_X - 1 && num_water; x++) {
      if (mapxy(x, y) == ter_water) {
        num_water--;
        fwrite(&x, 1, 1, f);
        fwrite(&y, 1, 1, f);
...
int write_lava(dungeon *d, FILE *f)
{
  uint16_t num_lava;
  uint8_t x, y;

  num_lava = htobe16(count_lava(d));
  fwrite(&num_lava, 2, 1, f);
  for (y = 1; y < DUNGEON_Y - 1 && num_lava; y++) {
    for (x = 1; x < DUNGEON_X - 1 && num_lava; x++) {
      if (mapxy(x, y) == ter_lava) {
        num_lava--;
        fwrite(&x, 1, 1, f);
        fwrite(&y, 1, 1, f);
...
int write_earthquack(dungeon *d, FILE *f)
{
  uint16_t num_earthquack;
  uint8_t x, y;

  num_earthquack = htobe16(count_earthquack(d));
  fwrite(&num_earthquack, 2, 1, f);
  for (y = 1; y < DUNGEON_Y - 1 && num_earthquack; y++) {
    for (x = 1; x < DUNGEON_X - 1 && num_earthquack; x++) {
      if (mapxy(x, y) == ter_earthquack) {
        num_earthquack--;
        fwrite(&x, 1, 1, f);
        fwrite(&y, 1, 1, f);
...
int read_water(dungeon *d, FILE *f)
{
  uint16_t num_water;
  uint8_t x, y;

  fread(&num_water, 2, 1, f);
  num_water = be16toh(num_water);
  for (; num_water; num_water--) {
    fread(&x, 1, 1, f);
    fread(&y, 1, 1, f);
    mapxy(x, y) = ter_water;
  }
...
int read_lava(dungeon *d, FILE *f)
{
  uint16_t num_lava;
  uint8_t x, y;

  fread(&num_lava, 2, 1, f);
  num_lava = be16toh(num_lava);
  for (; num_lava; num_lava--) {
    fread(&x, 1, 1, f);
    fread(&y, 1, 1, f);
    mapxy(x, y) = ter_lava;
  }
...
int read_eathquack(dungeon *d, FILE *f)
{
  uint16_t num_earthquack;
  uint8_t x, y;

  fread(&num_earthquack, 2, 1, f);
  num_earthquack = be16toh(num_earthquack);
  for (; num_earthquack; num_earthquack--) {
    fread(&x, 1, 1, f);
    fread(&y, 1, 1, f);
    mapxy(x, y) = ter_earthquack;
  }
