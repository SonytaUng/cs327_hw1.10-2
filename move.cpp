...
uint32_t move_pc(dungeon *d, uint32_t dir)
{
  ...
  uint32_t was_water = 0;
  uint32_t was_lava = 0;
  uint32_t was_earthquack = 0;
  const char *wallmsg[] = {
  ...
  case '>':
    ...
    break;
  case 'W':
    if (mappair(d->PC->position) == ter_water) {
      was_water = 1;
      new_dungeon_level(d, 'W');
      //pc lost equipment 25%
    }
    break;
  case 'L':
    if (mappair(d->PC->position) == ter_lava) {
      was_lava = 1;
      new_dungeon_level(d, 'L');
      //pc lost equipent 50%
    }
    break;
  case 'E':
    if (mappair(d->PC->position) == ter_earthquack) {
      was_earthquack = 1;
      new_dungeon_level(d, 'E');
      //pc lost equipment 75%
    }
    break;
  }
  ...
  if (was_earthquack) {
   ...
   if ((dir != '>') && (dir != '<') && (dir != 'W') && (dir != 'L') && (dir != 'E') && (mappair(next) >= ter_floor)) {
