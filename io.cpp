  ...
  for (pos[dim_y] = -PC_VISUAL_RANGE;
    ...
    for (pos[dim_x] = -PC_VISUAL_RANGE;
      ...
      if (cursor[dim_y] == d->PC->position[dim_y] + pos[dim_y] &&
        ...
        case ter_water:
          mvaddch(d->PC->position[dim_y] + pos[dim_y] + 1,
                  d->PC->position[dim_x] + pos[dim_x], 'W');
          break;
        case ter_lava:
          mvaddch(d->PC->position[dim_y] + pos[dim_y] + 1,
                  d->PC->position[dim_x] + pos[dim_x], 'L');
          break;
        case ter_earthquack:
          mvaddch(d->PC->position[dim_y] + pos[dim_y] + 1,
                  d->PC->position[dim_x] + pos[dim_x], 'E');
          break;
        default:
      ...
      if (d->character_map[pos[dim_y]]
        ...
        switch (pc_learned_terrain(d->PC,
          ...
          case ter_stairs_down:
            ...
            break;
          case ter_water:
            mvaddch(pos[dim_y] + 1, pos[dim_x], 'W');
            count++;
            break;
          case ter_lava:
            mvaddch(pos[dim_y] + 1, pos[dim_x], 'L');
            count++;
            break;
          case ter_earthquack:
            mvaddch(pos[dim_y] + 1, pos[dim_x], 'E');
            count++;
            break;
          default:
        /* Use zero as an error symbol, since it stands out somewhat, and it's *
...
uint32_t io_teleport_pc(dungeon *d)
{
  ...
  do {
    ...
    case ter_stairs_down:
      ...
      break;
    case ter_water:
      mvaddch(dest[dim_y] + 1, dest[dim_x], 'W');
      break;
    case ter_lava:
      mvaddch(dest[dim_y] + 1, dest[dim_x], 'L');
      break;
    case ter_earthquack:
      mvaddch(dest[dim_y] + 1, dest[dim_x], 'E');
      break;
    default:
 /* Use zero as an error symbol, since it stands out somewhat, and it's *
  * not otherwise used.                                                 */
